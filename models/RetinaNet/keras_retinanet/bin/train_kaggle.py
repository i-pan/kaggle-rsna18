...
from ..utils.evaluate_with_kaggle_metric import KaggleEvaluate 
...
def create_callbacks(epoch, model, training_model, prediction_model, args):
...
def main(args=None):
    ...
    if not os.path.exists(args.snapshot_path): os.makedirs(args.snapshot_path)
    if not os.path.exists(args.snapshot_path + "/results/"): os.makedirs(args.snapshot_path + "/results/")
    ...
    for e in range(args.epochs):
    	...
    	model_save_path = os.path.join(args.snapshot_path,
                                       '{}_{}_{}.h5'.format(args.backbone, 
                                                            args.dataset_type, 
                                                            str(e).zfill(2)))
        results_save_path = os.path.join(args.snapshot_path, 
                                         'results', 
                                         'results_{}_{}_epoch{}.csv'.format(args.backbone, 
                                                                            args.dataset_type, 
                                                                            str(e).zfill(2)))
        # Tuple of 3: (max mAP over all images, 
        #              max mAP over positive images,
        #              overall AUROC)
        metrics = KaggleEvaluate(model_save_path, 
                                 results_save_path, 
                                 args.backbone, 
		                         args.val_annotations, 
                                 args.stratified_folds, 
                                 args.fold, 
                                 args.image_min_side, 
                                 args.data_dir)
        if args.mode == "detector": 
            ...
            eval_metric = metrics[2] 
        # If validation metric does not improve, reduce learning rate by factor of 10
        # if eval_metric > best_eval_metric: 
        #    best_eval_metric = eval_metric
        #else: 
        if e == 3 or e == 6:
            K.set_value(training_model.optimizer.lr, K.get_value(training_model.optimizer.lr) / 10.) 
